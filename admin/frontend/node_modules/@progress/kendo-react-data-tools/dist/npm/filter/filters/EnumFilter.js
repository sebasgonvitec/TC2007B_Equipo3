"use strict";
var __extends = (this && this.__extends) || (function () {
    var extendStatics = function (d, b) {
        extendStatics = Object.setPrototypeOf ||
            ({ __proto__: [] } instanceof Array && function (d, b) { d.__proto__ = b; }) ||
            function (d, b) { for (var p in b) if (Object.prototype.hasOwnProperty.call(b, p)) d[p] = b[p]; };
        return extendStatics(d, b);
    };
    return function (d, b) {
        if (typeof b !== "function" && b !== null)
            throw new TypeError("Class extends value " + String(b) + " is not a constructor or null");
        extendStatics(d, b);
        function __() { this.constructor = d; }
        d.prototype = b === null ? Object.create(b) : (__.prototype = b.prototype, new __());
    };
})();
var __assign = (this && this.__assign) || function () {
    __assign = Object.assign || function(t) {
        for (var s, i = 1, n = arguments.length; i < n; i++) {
            s = arguments[i];
            for (var p in s) if (Object.prototype.hasOwnProperty.call(s, p))
                t[p] = s[p];
        }
        return t;
    };
    return __assign.apply(this, arguments);
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.EnumFilter = void 0;
var React = require("react");
var PropTypes = require("prop-types");
var kendo_react_dropdowns_1 = require("@progress/kendo-react-dropdowns");
var kendo_react_intl_1 = require("@progress/kendo-react-intl");
var messages_1 = require("./../../messages");
/**
 * The EnumFilter component used for editing the value of FilterDescriptor object.
 * It renders a DropDownList and allows you to choose a value from the predefined list.
 */
var EnumFilter = /** @class */ (function (_super) {
    __extends(EnumFilter, _super);
    function EnumFilter() {
        var _this = _super !== null && _super.apply(this, arguments) || this;
        _this.onChange = function (event) {
            _this.props.onFilterChange.call(undefined, {
                nextFilter: __assign(__assign({}, _this.props.filter), { value: event.value.value })
            });
        };
        return _this;
    }
    /**
     * @hidden
     */
    EnumFilter.prototype.render = function () {
        var locService = (0, kendo_react_intl_1.provideLocalizationService)(this);
        var _a = this.props, filter = _a.filter, _b = _a.data, data = _b === void 0 ? [] : _b, defaultItem = _a.defaultItem, _c = _a.ariaLabel, ariaLabel = _c === void 0 ? locService.toLanguageString(messages_1.filterEnumFilterDropdownAriaLabel, messages_1.messages[messages_1.filterEnumFilterDropdownAriaLabel]) : _c;
        var locData = data.map(function (item) { return (__assign(__assign({}, item), { text: locService.toLanguageString(item.text, messages_1.messages[item.text] || item.text) })); });
        return (React.createElement(kendo_react_dropdowns_1.DropDownList, { value: locData.find(function (i) { return i.value === filter.value; }) || null, onChange: this.onChange, defaultItem: defaultItem, data: locData, textField: "text", ariaLabel: ariaLabel }));
    };
    /**
     * @hidden
     */
    EnumFilter.propTypes = {
        filter: PropTypes.object.isRequired,
        onFilterChange: PropTypes.func.isRequired,
        data: PropTypes.arrayOf(PropTypes.shape({ text: PropTypes.string, value: PropTypes.any })),
        defaultItem: PropTypes.any,
        ariaLabel: PropTypes.string
    };
    return EnumFilter;
}(React.Component));
exports.EnumFilter = EnumFilter;
(0, kendo_react_intl_1.registerForLocalization)(EnumFilter);
